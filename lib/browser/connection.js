...
var Connection = (module.exports = function (options) {
    ...
    var batchCacheOptions = {
        key: function (options) {
            ...
            var typesToFetch =
                types.length > MAX_BATCH_REQUESTS
                    ? autofetch
                        ? types
                        : types.slice(0, MAX_BATCH_REQUESTS)
                    : types;
            var keys = [];
    ...
    this.batchDescribe = this.cache.makeResponseCacheable(
        this.batchDescribe,
        this,
        batchCacheOptions
    );
...
Connection.prototype._retrieveParallel = function (type, ids, options) {
    ...
    return Promise.all(
        ids.map(function (id) {
            return self
                ._retrieveSingle(type, id, options)
                .catch(function (err) {
 ...
 *
 * @method Connection#getUser
 * @return {Promise<Record>}
 */
Connection.prototype.getUser = function () {
    return this._retrieveSingle("User", this.userInfo.id, { fields: null });
};
...
Connection.prototype._createParallel = function (type, records, options) {
    ...
    return Promise.all(
        records.map(function (record) {
            return self
                ._createSingle(type, record, options)
                .catch(function (err) {
...
Connection.prototype._createMany = function (type, records, options) {
    ...
    records = _.map(records, function (record) {
        var sobjectType =
            type ||
            (record.attributes && record.attributes.type) ||
            record.type;
        if (!sobjectType) {
            return Promise.reject(
...
Connection.prototype._updateParallel = function (type, records, options) {
    ...
    return Promise.all(
        records.map(function (record) {
            return self
                ._updateSingle(type, record, options)
                .catch(function (err) {
                    // be aware that allOrNone in parallel mode will not revert the other successful requests
                    // it only raises error when met at least one failed request.
                    if (options.allOrNone || !err.errorCode) {
...
Connection.prototype["delete"] =
    ...
    Connection.prototype.destroy =
        function (type, ids, options, callback) {
            if (typeof options === "function") {
                callback = options;
                options = {};
            }
 ...
 * @returns {Promise.<DescribeSObjectResult>}
 */
Connection.prototype.describe = Connection.prototype.describeSObject =
 ...
 * @returns {Promise.<DescribeSObjectResult[]>}
 */
Connection.prototype.batchDescribe =
    Connection.prototype.batchDescribeSObjects = function (options, callback) {
        var self = this;
        var types = options.types;
        var autofetch = options.autofetch || false;
        var maxConcurrentRequests = Math.min(
        ...
        do {
            ...
            types =
                types.length > MAX_BATCH_REQUESTS
                    ? types.slice(MAX_BATCH_REQUESTS)
                    : [];
        } while (types.length > 0 && autofetch);
        var requestBatches = [];
        do {
            var requestBatch =
                batches.length > maxConcurrentRequests
                    ? batches.slice(0, maxConcurrentRequests)
                    : batches;
            requestBatches.push(requestBatch);
            batches =
                batches.length > maxConcurrentRequests
                    ? batches.slice(maxConcurrentRequests)
                    : [];
        } while (batches.length > 0);
        return self
            .doBatchDescribeRequestBatches(requestBatches)
            .thenCall(callback);
    };
...
Connection.prototype.doBatchDescribeRequestBatches = function (requestBatches) {
    ...
    return self
        .doBatchOfBatchDescribeRequests(firstBatch)
        .then(function (sobjectArray) {
        ...
        if (response.results) {
            ...
            for (var i = 0; i < response.results.length; i++) {
                ...
                if (Array.isArray(subResp.result)) {
                    if (
                        subResp.result[0].errorCode &&
                        subResp.result[0].message
                    ) {
                        this._logger.error(
                            "Error: " +
                                subResp.result[0].errorCode +
...
/**
 * Get the identity url for this org
 * @returns {String}
 */
Connection.prototype._getIdentityUrl = function () {
    return this.loginUrl + "/services/oauth2/userinfo";
};
...
Connection.prototype.identity = function (options, callback) {
    ...
    var self = this;
    return Promise.resolve(
        this.request({ method: "GET", url: this._getIdentityUrl() })
    )
        .then(function (res) {
            self.userInfo = {
                id: res.user_id,
